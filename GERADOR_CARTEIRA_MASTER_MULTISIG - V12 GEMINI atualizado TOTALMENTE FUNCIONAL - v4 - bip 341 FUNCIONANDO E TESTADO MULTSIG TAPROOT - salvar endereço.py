...
"""
GERADOR DE CARTEIRAS BITCOIN OFFLINE - v2.4 (Final) - MODIFICADO
================================================================
...
"""
    ...
    # Cria um arquivo .txt com informações sobre como obter o endereço da carteira.
    address_info_filename = OUTPUT_DIR / f"{name}_address_info.txt"
    address_info_content = f"""
INFORMAÇÃO SOBRE O ENDEREÇO DA CARTEIRA
========================================

Carteira: '{name}' (Multisig Taproot - Avançado Miniscript)

Este script gerou com sucesso um "Output Descriptor" para a sua carteira.
O descriptor é a "chave mestra" que contém todas as informações para que uma
carteira de software (como Sparrow, Specter, Electrum) possa descobrir
todas as suas chaves, gerar seus endereços e criar transações.

Como visualizar seus endereços:
-------------------------------
Devido à natureza avançada desta carteira (Taproot com Miniscript), o endereço
não é pré-calculado por este script. Para ver seus endereços de recebimento,
você DEVE importar o descriptor em uma carteira de software compatível.

1. Localize o arquivo de descriptor: {name}_descriptor.txt
   (Ou o arquivo de importação para Sparrow: {name}.bsms)

2. Importe este arquivo na sua carteira de software preferida (ex: Sparrow Wallet).

3. A carteira usará o descriptor para gerar e exibir todos os seus endereços
   de recebimento de forma segura. O primeiro endereço será exibido na aba
   "Addresses".

Não tente criar um endereço manualmente. Use sempre a carteira de software
com o descriptor importado para garantir que os fundos sejam enviados
corretamente para sua custódia.
"""
    address_info_filename.write_text(address_info_content, encoding="utf-8")
    print(f"  - Arquivo de informação sobre o endereço salvo em: {address_info_filename.name}")
    # --- FIM DA MODIFICAÇÃO ---
